        ...
        <div class="container-fluid">
          ...
          <div class="card bg-company mb-4 row ml-1 mr-1">
            <div class="card-body p-3">
              <h5 class="m-0 font-weight-bold text-white"><a href="<?= site_url('perusahaan/data_rembes') ?>" class="text-white"><i class="fas fa-arrow-alt-circle-left mr-3" style="font-size: 20px;" data-toggle="tooltip" data-placement="bottom" title="Kembali"></i></a>Detail Rembes <?= $total_rembes < $uang_lumpsum ? '<p class="kat-purple ml-3">Nota</p>':'<p class="kat-orange ml-3">Pengajuan</p>' ?></h5>
            </div>
          </div>
          ...
          <div class="card shadow mb-4 mt-4">
            <div class="card-header bg-company py-3">
              <h6 class="m-0 font-weight-bold text-white float-left">List Nota Rembes</h6>
            </div>
            <div class="card-body">
              <div class="table-responsive">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <?php $getRek = $this->db->query('SELECT * FROM tb_master_rembes WHERE id_master_rembes="'.$id_master_rembes.'"')->row(); ?>
        <table class="table table-sm table-borderless table-condensed">
          <tbody>
            <tr>
              <td width="35%">Jenis Bank</td>
              <td><p><?= $getRek->jenis_bank ?></p></td>
            </tr>
            <tr>
              <td width="35%">Nama Rekening</td>
              <td><p><?= $getRek->nama_rekening ?></p></td>
            </tr>
            <tr>
              <td width="35%">No Rekening</td>
              <td><p><?= $getRek->no_rekening ?></p></td>
            </tr>
          </tbody>
        </table>
      </div>
...
<script type="text/javascript">
  $(document ).ready(function(){
    ...
    $('#dataTable').DataTable({
      ...
      "order":[],
      "ajax":{
        url:"<?= site_url('perusahaan/data_rembes/list_rembes/fetch') ?>",
        type:"POST",
        data:function(data){
